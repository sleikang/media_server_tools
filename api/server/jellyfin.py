...
class jellyfin(serverbase):
    ...
    def get_items(self, parentid : str = '', type = None):
        ...
        try:
            if len(parentid):
                url = '{}/Users/{}/Items?ParentId={}&api_key={}'.format(self.host, self.userid, parentid, self.key)
            else:
                url = '{}/Users/{}/Items?api_key={}'.format(self.host, self.userid, self.key)
    ...
    def get_items_count(self):
        ...
        iteminfo = {}
        try:
            url = '{}/Items/Counts?api_key={}'.format(self.host, self.key)
            p, err = self.client.get(url=url)
            if not self.__get_status__(p=p, err=err):
    ...
    def get_item_info(self, itemid : str):
        ...
        iteminfo = {}
        try:
            url = '{}/Users/{}/Items/{}?Fields=ChannelMappingInfo&api_key={}'.format(self.host, self.userid, itemid, self.key)
            p, err = self.client.get(url=url)
            if not self.__get_status__(p=p, err=err):
                return False, iteminfo
            iteminfo = json.loads(p.text)
    ...
    def set_item_info(self, itemid : str, iteminfo):
        ...
        """
        try:
            url = '{}/Items/{}?api_key={}'.format(self.host, itemid, self.key)
    ...
    def search_movie(self, itemid, tmdbid, name = None, year = None):
        ...
        iteminfo = {}
        try:
            url = '{}/Items/RemoteSearch/Movie?api_key={}'.format(self.host, self.key)
    ...
    def apply_search(self, itemid, iteminfo):
        ...
        """
        try:
            url = '{}/Items/RemoteSearch/Apply/{}?ReplaceAllImages=true&api_key={}'.format(self.host, itemid, self.key)
            data = json.dumps(iteminfo)
            p, err = self.client.post(url=url, headers=self.headers, data=data)
            if not self.__get_status__(p=p, err=err):
                return False
    ...
    def refresh(self, itemid):
        ...
        """
        try:
            url = '{}/Items/{}/Refresh?Recursive=true&ImageRefreshMode=FullRefresh&MetadataRefreshMode=FullRefresh&ReplaceAllImages=true&ReplaceAllMetadata=true&api_key={}'.format(self.host, itemid, self.key)
            p, err = self.client.post(url=url, headers=self.headers)
            if not self.__get_status__(p=p, err=err):
                return False
            return True
