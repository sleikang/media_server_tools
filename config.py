    ...
    def __init__(self, path : str) -> None:
        try:
            ...
            if not os.path.exists(path=self.path):
                ...
                data = {'system':{'embyhost':'', 'embyuserid':'', 'embykey':'', 'tmdbkey':'', 'doubankey':'0ac44ae016490db2204ce0a042db2916', 'doubancookie':'', 'threadnum':1, 'updatepeople':True, 'updateoverview':True, 'updateseasongroup':True, 'updatetime':1, 'taskdonespace':1, 'delnotimagepeople':True, 'cachefailtime':30}}
            ...
            self.__config_check__('updateseasongroup', True)
            self.__config_check__('updatetime', 1)
            self.__config_check__('taskdonespace', 1)
            self.__config_check__('delnotimagepeople', True)
